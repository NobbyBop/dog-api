...
const listBreedsRoute = createRoute({
  ...
  description: 'Retrieve a list of all available dog breeds with their characteristics',
  request: {
    query: z.object({
      group: z.enum(['sporting', 'hound', 'working', 'terrier', 'toy', 'non-sporting', 'herding', 'mixed']).optional(),
      size: z.enum(['small', 'medium', 'large', 'extra-large']).optional(),
      exerciseNeeds: z.enum(['low', 'moderate', 'high', 'very-high']).optional(),
...
breedsRoutes.openapi(listBreedsRoute, (c) => {
  ...
  if (query.group) {
    filteredBreeds = filteredBreeds.filter(breed => breed.group === query.group)
  }
  if (query.size) {
    filteredBreeds = filteredBreeds.filter(breed => breed.size === query.size)
  }
  if (query.exerciseNeeds) {
    filteredBreeds = filteredBreeds.filter(breed => breed.exerciseNeeds === query.exerciseNeeds)
  }
  if (query.goodWithKids !== undefined) {
    filteredBreeds = filteredBreeds.filter(breed => breed.goodWithKids === query.goodWithKids)
  }
  if (query.goodWithPets !== undefined) {
    filteredBreeds = filteredBreeds.filter(breed => breed.goodWithPets === query.goodWithPets)
  }
...
breedsRoutes.openapi(getBreedGroupsRoute, (c) => {
  ...
  const groupDescriptions = {
    ...
    'working': 'Dogs bred for specific jobs like guarding, pulling sleds, or water rescue',
    'terrier': 'Dogs bred to hunt and kill vermin',
    'toy': 'Small companion dogs bred primarily for companionship',
    'non-sporting': 'Diverse group of dogs that don\'t fit into other categories',
  ...
  const groups = Object.entries(groupCounts).map(([name, count]) => ({
    name,
    description: groupDescriptions[name as keyof typeof groupDescriptions] || 'Mixed breed dogs',
    count
  }))
