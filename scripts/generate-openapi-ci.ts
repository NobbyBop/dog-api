  ...
  try {
    ...
    const openAPIDocument = app.getOpenAPIDocument({
      ...
      info: {
        version: '1.0.0',
        title: 'Dog API',
        description: 'A comprehensive API for managing dogs, breeds, adoption, health records, and training data'
      }
    })
    ...
    console.log(`🏷️  API version: ${openAPIDocument.info?.version || 'unknown'}`)
    ...
    if (process.env.GITHUB_ACTIONS) {
      ...
      console.log(`::set-output name=api-version::${openAPIDocument.info?.version || 'unknown'}`)
      console.log(`::set-output name=generated-at::${schemaWithMetadata['x-generated-at']}`)
    }
